...
export const getCharacter = name => {
  const url = `${advertsBaseUrl}/characters?name=${name}&apikey=${process.env.REACT_APP_API_APIKEY}&ts=${process.env.REACT_APP_API_TS}&hash=${process.env.REACT_APP_API_HASH}`;
  return client.get(url);
};
